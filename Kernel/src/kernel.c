	...
	if ((fid = fopen(script,"r+")) == NULL)
  	{
		...
		printf("Longitud del parametro ruta del archivo: %d",string_length(script));
     		return;
  	}
...
void *exec(void * numero_exec)
{
	...
	while (1) {
		...
		printf("**** El hilo de ejecucion %d va a ejecutar el siguiente request ****\n",thread_n);
...
void *agregar_a_ready()
{
	while(1)
	{
		...
		t_request_struct requests_struct = crear_estructura_para_planificar((t_queue*) requests);
...
t_queue* armar_request_queue(char * input)
{
	...
	if (string_starts_with(input,"@")) {
		procesar_script(string_substring(input,1,string_length(input)-3),request_queue);
		return request_queue;
	}
...
int procesar()
{
	...
	for(p = res; p != NULL; p = p->ai_next) {
		...
		if ((sckt = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1) {
...
int main(int argc, char *argv[])
{
	...
	iniciar_estructuras();solicitar_memorias();
